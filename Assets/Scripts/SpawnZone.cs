...
{
    [System.Serializable]
    public struct SpawnConfiguartion
    {
        public enum MovementDirection { Forward, Upward, Outward, Random }
    ...
    [SerializeField]
    SpawnConfiguartion spawnConfig;
    ...
    public virtual void SpawnShapes()
    {
        ...
        {
            CreateSatelliteFor(shape, spawnConfig.lifecycle.uniformLifecycles ? lifecycleDurations : spawnConfig.lifecycle.RandomDurations);
        }
    ...
    void CreateSatelliteFor(Shape focalShape, Vector3 lifecycleDurations)
    {
        ...
        SetupColor(shape);
        shape.AddBehavior<SatelliteBehavior>().Initialize(
            shape, focalShape,
    ...
    void SetupOscillation(Shape shape)
    {
        ...
        oscillation.Offset = amplitude * GetDirectionVector(spawnConfig.oscillationDirection, shape.transform);
    }
    ...
    private Vector3 GetDirectionVector(SpawnConfiguartion.MovementDirection direction, Transform t)
    {
        switch (direction)
        {
            case SpawnConfiguartion.MovementDirection.Upward:
                return transform.up;
            case SpawnConfiguartion.MovementDirection.Outward:
                return (t.localPosition - transform.position).normalized;
            case SpawnConfiguartion.MovementDirection.Random:
